...
$(function() {
	...
	$('.js_main_section_visual_play_open').on('click', function(event) {
		...
		if ( agent.devicename.search('LG-F310') == 0 ) {
			$('.js-food-play-box').css({'display':'table','visibility':'visible'});
		} else {
			$('.js-food-play-box').show().css({'visibility':'visible'});
		}
		...
		if($('html').hasClass('csstransforms') && !$('body').hasClass('ui-browser-ie')) {
			...
			TweenMax.to(playerEl, .5, {scale:1, y:'-50%', opacity:1, ease:Power2.easeOut});
	...
	$('.js-food-play-close').on('click', function() {
		$('.js-food-play-box').hide().css({'visibility':'hidden'});
